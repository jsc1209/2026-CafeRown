...
public class CustomerServiceImpl implements CustomerService {
    ...
    public int getCountByGrade(String grade) {
        return customerMapper.getCountByGrade(grade);
    }

    @Override
    public int getCountThisMonth() {
        return customerMapper.getCountThisMonth();
    }

    @Override
    public long getTotalMonthlyAmount() {
        return customerMapper.getTotalMonthlyAmount();
    }

    @Override
    public int getTotalMonthlyVisit() {
        return customerMapper.getTotalMonthlyVisit();
    }
     ...
     *
     * 등급 기준 (이번 달 누적 기준):
     * VIP : 월 30회+ 방문 → 결제액의 10% 적립
     * 골드 : 월 15회+ 방문 OR 7만원+ → 결제액의 7% 적립
     * 실버 : 월 5회+ 방문 OR 3만원+ → 결제액의 5% 적립
    ...
    public int addVisitAndUpdateGrade(String c_idx, int amount, String menuItem, String note) {
        ...
        if (result > 0) {
            ...
            if (customer == null)
                return result; // 삭제된 고객 또는 잘못된 c_idx 방어
            ...
            if (mv >= 30)
                newGrade = "VIP";
            else if (mv >= 15 || ma >= 70_000)
                newGrade = "골드";
            else if (mv >= 5 || ma >= 30_000)
                newGrade = "실버";
            else
                newGrade = "일반";
    ...
    // ── 방문 이력 ──────────────────────────────────────────────
    @Override
    public List<VisitLogVO> getVisitLogs(String c_idx) {
        return customerMapper.getVisitLogs(c_idx);
    }

    // ── 태그 ───────────────────────────────────────────────────
    @Override
    public List<String> getTags(String c_idx) {
        return customerMapper.getTags(c_idx);
    }
    ...
    // ── 마케팅용 ───────────────────────────────────────────────
    @Override
    public List<CustomerVO> getInactiveCustomers(int days) {
        return customerMapper.getInactiveCustomers(days);
    }

    @Override
    public List<CustomerVO> getBirthdayCustomers() {
        return customerMapper.getBirthdayCustomers();
    }

    // ── 통계 ───────────────────────────────────────────────────
    @Override
    public List<Map<String, Object>> getGradeDistribution() {
        return customerMapper.getGradeDistribution();
    }

    @Override
    public List<Map<String, Object>> getMonthlyNewCustomers() {
        return customerMapper.getMonthlyNewCustomers();
    }

    @Override
    public List<Map<String, Object>> getVisitDistribution() {
        return customerMapper.getVisitDistribution();
    }

    @Override
    public List<CustomerVO> getTopByVisit(int limit) {
        return customerMapper.getTopByVisit(limit);
    }

    @Override
    public double getAvgVisitCount() {
        return customerMapper.getAvgVisitCount();
    }

    @Override
    public List<CustomerVO> getCustomerProgress() {
        return customerMapper.getCustomerProgress();
    }

    @Override
    public List<Map<String, Object>> getHourlyStats() {
        return customerMapper.getHourlyStats();
    }

    @Override
    public List<Map<String, Object>> getWeekdayStats() {
        return customerMapper.getWeekdayStats();
    }

    @Override
    public List<Map<String, Object>> getTopMenuItems(int lim) {
        return customerMapper.getTopMenuItems(lim);
    }

    @Override
    public List<Map<String, Object>> getGradeAvgSpend() {
        return customerMapper.getGradeAvgSpend();
    }

    @Override
    public List<CustomerVO> getAllCustomersForExport() {
        return customerMapper.getAllCustomersForExport();
    }
}
