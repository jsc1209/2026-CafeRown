      ...
      <div class="nav-auth">
        <c:choose>
          <c:when test="${not empty sessionScope.m_id}">
            <button type="button" class="cart-trigger" onclick="openCart()" aria-label="장바구니 열기">
              🛒 <span class="cart-badge" id="cart-badge" style="display:none">0</span>
            </button>
            <div class="nav-dropdown">
              <button class="user-btn" onclick="toggleUserDropdown()">👤 ${sessionScope.m_name} ▾</button>
              <div class="user-dropdown" id="user-dropdown">
                <a href="${pageContext.request.contextPath}/member/mypage" class="dropdown-item">마이페이지</a>
...
<%-- 사이드 슬라이드 장바구니 패널 (로그인 회원 전용 — 내부에서 c:if 처리) --%>
<%@ include file="cart-panel.jsp" %>
