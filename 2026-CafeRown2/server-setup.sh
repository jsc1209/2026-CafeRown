...
# 기존 프로세스 종료
pkill -f brew-crm.war 2>/dev/null || true

# 포트 8081 점유 프로세스 강제 종료
for i in 1 2 3 4 5; do
    PIDS=$(lsof -i :8081 -t 2>/dev/null || true)
    if [ -z "$PIDS" ]; then
        break
    fi
    echo "  → 포트 8081 점유 프로세스 종료 중... (시도 $i)"
    kill -9 $PIDS 2>/dev/null || true
    sleep 2
done
echo "  → 기존 프로세스 종료 완료."

# Tomcat JSP 컴파일 캐시 삭제 (핵심: 이게 없으면 옛날 JSP가 계속 나옴)
echo "  → Tomcat 캐시 디렉토리 삭제 중..."
rm -rf /tmp/tomcat* 2>/dev/null || true
rm -rf /tmp/undertow* 2>/dev/null || true
echo "  → 캐시 삭제 완료."

# 새 앱 시작
nohup java \
    -Dserver.tomcat.basedir=/home/ubuntu/tomcat-work \
    -jar /home/ubuntu/brew-crm.war \
